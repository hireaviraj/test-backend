# vychron EKS Deployment Guide — test-backend
...
- **GitOps repo** — `hireaviraj/hireaviraj-gitops-config` contains your Helm chart and ArgoCD manifests
...
```json
{
  ...
  "Statement": [{
    ...
    "Condition": {
      "StringLike": {
        "token.actions.githubusercontent.com:sub": "repo:hireaviraj/test-backend:ref:refs/heads/main"
      },
      "StringEquals": {
...
This trust policy only allows the `main` branch of **hireaviraj/test-backend** to assume the role.
...
ECR login → docker build → docker push (tag: sha-<commit>)
  ↓
Clone hireaviraj/hireaviraj-gitops-config
  ↓
python bin/update-image-tag.py --service <service> --tag sha-<commit>
  ↓
git push → gitops repo updated
  ↓
...
- If stuck: `argocd app sync test-backend` manually
